  ...
  async (messageData, { rejectWithValue }) => {
    ...
    try {
      const messageRecieved = await axios.post(
        `http://localhost:8005/main/messages/sendmessage`,
        messageData,
  ...
  async ({ receiverId, page, tokenUserId }, { rejectWithValue }) => {
    ...
    try {
      const allSentMessages = await axios.get(
        `http://localhost:8005/main/messages/getsendermessage?receiverId=${receiverId}&page=${page}&size=6`,
        {
          headers: {
  ...
  async ({ postId, checkedUser }, { rejectWithValue }) => {
    ...
    try {
      const updatedValues = await axios.post(
        `http://localhost:8005/main/messages/sendposttousers`,
        {
          postId: postId,
