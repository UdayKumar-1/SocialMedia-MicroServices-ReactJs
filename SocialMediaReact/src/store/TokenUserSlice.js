  ...
  async (_, { rejectWithValue }) => {
    ...
    try {
      const user = await axios.get(
        `http://localhost:8005/main/users/tokenUser`,
        {
          headers: {
  ...
  async (_, { rejectWithValue }) => {
    ...
    try {
      const postCount = await axios.get(
        `http://localhost:8005/main/posts/postscount`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  async (fileData, { rejectWithValue }) => {
    ...
    try {
      const updatedUserProfile = await axios.put(
        `http://localhost:8005/main/users/allUsers/update`,
        fileData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
...
export const followUser = createAsyncThunk(
  ...
  async (userId, { rejectWithValue }) => {
    const token = localStorage.getItem("token");
    try {
      const followUserData = await axios.post(
        `http://localhost:8005/main/users/allUsers/follow?userId=${userId}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
...
export const tokenUserEditProfile = createAsyncThunk(
  ...
  async (editProfileData, { rejectWithValue }) => {
    ...
    try {
      const updatedUserBackground = await axios.put(
        `http://localhost:8005/main/users/edit`,
        editProfileData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
