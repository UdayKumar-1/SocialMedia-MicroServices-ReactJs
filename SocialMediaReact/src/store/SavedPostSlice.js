...
export const savePostThunk = createAsyncThunk(
  ...
  async (savePostData, { rejectWithValue }) => {
    ...
    try {
      const savedPostData = await axios.post(
        `http://localhost:8005/main/savedposts/save`,
        savePostData,
        {
          headers: {
  ...
  async ({ postId }, { rejectWithValue }) => {
    ...
    try {
      const savedPostData = await axios.delete(
        `http://localhost:8005/main/savedposts/deleteSaved?postId=${postId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  async (likedPostId, { rejectWithValue }) => {
    ...
    try {
      const likeSuccessData = await axios.put(
        `http://localhost:8005/main/posts/liked`,
        likedPostId,
        {
          headers: {
            Authorization: `Bearer ${token}`,
