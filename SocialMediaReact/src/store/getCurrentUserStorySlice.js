  ...
  async (_, { rejectWithValue }) => {
    ...
    try {
      const currentStory = await axios.get(
        `http://localhost:8005/main/story/currentStory`,
        {
          headers: {
  ...
  async (storyInput, { rejectWithValue }) => {
    ...
    try {
      const storiesData = await axios.post(
        `http://localhost:8005/main/story/allStory`,
        storyInput,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  async (storyData, { rejectWithValue }) => {
    ...
    try {
      const currentStory = await axios.put(
        `http://localhost:8005/main/story/updateStory`,
        storyData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  async (_, { rejectWithValue }) => {
    ...
    try {
      const currentStory = await axios.delete(
        `http://localhost:8005/main/story/deleteStory`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
